...
@Sammy.on(events.CallbackQuery(pattern=r"check-bot-(\d+)"))
...
@Sammy.on(events.ChatAction)
async def join(event):

    if event.user_joined:
        await Sammy.edit_permissions(event.chat_id, event.user_id, send_messages=False)
