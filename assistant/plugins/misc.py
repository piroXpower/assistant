...
@Sammy.on(events.NewMessage(pattern="^[!?/]id"))
async def id(event):
...
@Sammy.on(events.NewMessage(pattern="^[!?/]info ?(.*)"))
async def info(event):

    sed = await Sammy(P(user_id=event.sender_id, offset=42, max_id=0, limit=80))
    hn = await Sammy(GetFullUserRequest(event.sender_id))
    text = "**✘ UserInfo:**\n\n"
    text += "**» Fɪʀsᴛ Nᴀᴍᴇ:** {}\n"
    text += "**» Lᴀsᴛ Nᴀᴍᴇ:** {}\n"
    text += "**» Usᴇʀ-ID:** `{}`\n"
    ...
    if not input_str:
          await Sammy.send_message(event.chat_id, text.format(hn.user.first_name, hn.user.last_name, event.sender_id, event.sender.username, sed.count, hn.about, event.sender_id))
          return
 
    input_str = event.pattern_match.group(1)
    ha = await Sammy.get_entity(input_str)
    hu = await Sammy(GetFullUserRequest(id=input_str))
    sedd = await Sammy(P(user_id=input_str, offset=42, max_id=0, limit=80))
...
@Sammy.on(events.callbackquery.CallbackQuery(data="misc"))
async def _(event):
